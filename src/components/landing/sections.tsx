  ...
  return (
    <div ref={ref}>
      <p className="font-display text-4xl lg:text-5xl font-semibold text-light-text">
        {prefix}
        {count >= 1000 ? count.toLocaleString('es-AR') : count}
        {suffix}
      </p>
...
export function LandingHeader({
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'pt-3' : 'pt-5'}`}
    >
...
export function HeroSection({
  ...
  return (
    <section className="relative pt-16 pb-24 lg:pt-20 lg:pb-28">
      <div
        className="absolute inset-x-0 top-0 h-[32rem] bg-[radial-gradient(circle_at_top_left,rgba(182,138,62,0.18),transparent_42%),radial-gradient(circle_at_top_right,rgba(93,133,179,0.14),transparent_28%)]"
        aria-hidden="true"
      />
        ...
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-12 items-center">
          <div className="reveal">
            ...
            <p className="mt-7 text-lg sm:text-xl leading-relaxed text-light-secondary max-w-2xl">
              AsistPro convierte conversaciones en organización real. Agenda citas, registra
              movimientos y lee el estado del mes desde un panel que se siente menos genérico y
              mucho más útil.
            </p>
                ...
                <div className="border-b border-dark-border px-4 py-3 flex items-center justify-between bg-dark-bg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs uppercase tracking-[0.18em] text-dark-secondary">
                    ...
                    <div className="space-y-3">
                      {chatMessages.slice(0, visibleCount).map((message, index) => (
                        <div
                          ...
                          className={`flex ${message.side === 'user' ? 'justify-end' : 'justify-start'} animate-message`}
                        >
                          <div
                  ...
                  <div className="p-4 bg-[linear-gradient(180deg,rgba(16,25,39,0.98),rgba(9,17,27,1))]">
                    <div className="rounded-2xl border border-dark-border bg-dark-bg/70 p-4">
                      ...
                      <div className="space-y-3">
                        {heroStatusItems.map(([label, value]) => (
                          <div
                            key={label}
                            className="flex items-center justify-between border-b border-dark-border-subtle pb-3 last:border-b-0 last:pb-0"
                          >
                    ...
                    <div className="mt-4 flex items-center gap-2 rounded-full bg-dark-elevated px-4 py-3">
                      ...
                      <span className="text-sm text-dark-secondary">
                        Dictar una nueva instrucción
                      </span>
                      <button
                        className="ml-auto w-8 h-8 rounded-full bg-dark-accent text-dark-bg flex items-center justify-center cursor-pointer"
                        aria-label="Enviar"
                      >
        ...
        <div className="grid lg:grid-cols-[0.95fr_1.05fr] gap-10 items-start mb-14">
          ...
          <div className="reveal reveal-delay-2">
            <p className="text-lg leading-relaxed text-light-secondary">
              ...
              estructura y presentar el resultado con una jerarquía que invite a decidir, no a
              buscar.
            </p>
          </div>
        </div>
              ...
              >
                <div className="flex items-center gap-1 mb-5">
                  {[...Array(testimonial.rating)].map((_, starIndex) => (
                    <Star
                      key={starIndex}
                      className="w-4 h-4 text-light-accent-dark fill-current"
                      aria-hidden="true"
                    />
                  ))}
                </div>
...
export function PricingSection({
  ...
  isAnnual: boolean;
  onOpenPlan: (plan: PricingPlan) => void;
  onToggleAnnual: () => void;
}) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14 reveal">
          ...
          <p className="mt-5 text-lg text-light-secondary max-w-2xl mx-auto">
            Los tres planes conservan el flujo actual. La diferencia está en profundidad, volumen
          ...
          <div className="mt-8 inline-flex items-center gap-4 rounded-full border border-light-border bg-[rgba(255,251,245,0.78)] px-4 py-2">
            <span
              className={`text-sm font-medium ${!isAnnual ? 'text-light-text' : 'text-light-muted'}`}
            >
              Mensual
            </span>
            <button
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {pricingPlans.map((plan, index) => (
            ...
            >
              ...
              <div className="mb-6">
                <p className="text-xs uppercase tracking-[0.22em] text-inherit/70 mb-3">
                  {plan.name}
                </p>
                <h3 className="font-display text-3xl font-semibold mb-2">{plan.name}</h3>
                <p
                  className={`${plan.popular ? 'text-dark-secondary' : 'text-light-secondary'} text-sm`}
                >
                  {plan.description}
                </p>
              </div>
              <div className="mb-8">
                <div className="flex items-baseline gap-2">
                  ...
                  <span
                    className={`${plan.popular ? 'text-dark-secondary' : 'text-light-muted'} text-sm`}
                  >
                    {plan.period}
                  </span>
                </div>
                {plan.savings && isAnnual && (
                  <p
                    className={`mt-2 text-xs font-medium ${plan.popular ? 'text-dark-accent-dark' : 'text-light-accent-dark'}`}
                  >
                    {plan.savings}
                  </p>
                )}
                <p
                  className={`mt-2 text-xs ${plan.popular ? 'text-dark-secondary' : 'text-light-secondary'}`}
                >
              ...
              <div className="space-y-3 mb-8">
                  ...
                  <div key={feature} className="flex items-start gap-3">
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center mt-0.5 ${
                        plan.popular
                          ? 'bg-dark-accent-light text-dark-accent-dark'
                          : 'bg-light-accent-light text-light-accent-dark'
                      }`}
                    >
                ...
                {plan.notIncluded.length > 0 && (
                  <>
                    <div
                      className={`pt-3 text-[11px] uppercase tracking-[0.22em] ${plan.popular ? 'text-dark-secondary' : 'text-light-muted'}`}
                    >
                      No incluye
                    </div>
                    {plan.notIncluded.map(feature => (
                      <div
                        key={feature}
                        className={`flex items-start gap-3 text-sm ${plan.popular ? 'text-dark-secondary' : 'text-light-muted'}`}
                      >
          ...
          <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-center">
            <div>
              ...
              <h2 className="mt-5 font-display text-4xl lg:text-5xl font-semibold leading-tight text-dark-text-primary">
                Si el producto organiza tu operación, la interfaz tiene que demostrarlo desde el
                primer segundo.
              </h2>
              <p className="mt-5 text-lg text-dark-secondary max-w-2xl">
                AsistPro ya resolvía el flujo. Ahora también comunica criterio, estructura y
                confianza.
              </p>
            </div>
                ...
                <ul className="space-y-3 text-sm text-light-secondary">
                  {column.links.map(link => (
                    <li key={link}>
                      <a
                        href="#"
                        className="hover:text-light-accent-dark transition-colors min-h-[44px] flex items-center"
                      >
                        {link}
                      </a>
                    </li>
