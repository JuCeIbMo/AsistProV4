...
export function MesaFinanzasView({ data, currency, appointments }: MesaFinanzasViewProps) {
  ...
  return (
    <section>
      {/* HEADER */}
      <header style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap', marginBottom: 24 }}>
        <div>
          <div style={{ ...mono, fontSize: 11, letterSpacing: '.14em', color: '#8a7c5e', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 9 }}>
            <span style={{ display: 'block', width: 18, height: 1, background: '#547552' }} />
            Resumen de caja · {data.month_label}
          </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <button className="mesa-btn-accent" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 18px', borderRadius: 9, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 600, background: '#C94E2C', color: '#fff', boxShadow: '0 6px 16px rgba(201,78,44,.3)' }}>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#7bdc8a', boxShadow: '0 0 0 3px rgba(123,220,138,.35)' }} /> Cobrar todo
          </button>
      ...
      <div style={{ display: 'flex', gap: 18, marginBottom: 28, flexWrap: 'wrap' }}>
        {kpis.map((k) => (
          ...
          >
            ...
            <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 30, lineHeight: 1.1, color: k.dark ? '#F5F1E8' : (k.valueColor || '#221f1b') }}>
              {k.value}
            </div>
            <div style={{ ...mono, fontSize: 11, color: k.hintColor, marginTop: 2 }}>{k.hint}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 28, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        {/* LEFT */}
        <div style={{ flex: 1.7, minWidth: 360, display: 'flex', flexDirection: 'column', gap: 30 }}>

          {/* BAR CHART */}
          <section style={{ position: 'relative', borderRadius: 8, padding: '24px 26px 20px', background: '#FBF8EC', backgroundImage: 'repeating-linear-gradient(0deg,rgba(102,82,181,.09) 0 1px,transparent 1px 22px),repeating-linear-gradient(90deg,rgba(102,82,181,.09) 0 1px,transparent 1px 22px)', backgroundSize: '22px 22px', boxShadow: '0 16px 34px rgba(70,55,28,.16), inset 0 1px 0 rgba(255,255,255,.6)', border: '1px solid #E6DCC2' }}>
            <div style={{ position: 'absolute', top: -12, left: 28, background: '#1A1816', color: '#F5F1E8', ...mono, fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', padding: '5px 14px', borderRadius: 4, transform: 'rotate(-1.5deg)', boxShadow: '0 5px 12px rgba(0,0,0,.28)' }}>
            ...
            {chartData.length === 0 ? (
              ...
              <>
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 14, height: 200, paddingTop: 18, borderBottom: '2px solid #1A1816' }}>
                  {chartData.map((d, i) => {
                    const val    = parseFloat(d.income) || 0;
                    const h      = Math.round((val / maxIncome) * 180);
                    const isLast = i === chartData.length - 1;
                    return (
                      <div key={`${d.year}-${d.month}`} style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', alignItems: 'center', gap: 6, height: '100%' }}>
                        <div style={{ ...mono, fontSize: 11, color: isLast ? '#C94E2C' : '#7a6e55', fontWeight: isLast ? 500 : 400 }}>{fmt(String(val))}</div>
                        <div style={{ width: '100%', maxWidth: 46, height: h, background: isLast ? 'linear-gradient(#E0673F,#C94E2C)' : 'linear-gradient(#7ba06d,#547552)', borderRadius: '3px 3px 0 0', boxShadow: isLast ? 'inset 0 2px 0 rgba(255,255,255,.3), 0 6px 14px rgba(201,78,44,.28)' : 'inset 0 2px 0 rgba(255,255,255,.25), 0 4px 10px rgba(70,55,28,.16)' }} />
                      </div>
                    );
                  })}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 14, marginTop: 9, ...mono, fontSize: 11, color: '#9a824a', textAlign: 'center' }}>
                  {chartData.map((d, i) => {
                    const isLast = i === chartData.length - 1;
                    return (
                ...
                {pendingAppts.map((a, i) => {
                  ...
                  return (
                    <div
                      key={a.id}
                      className="mesa-lift"
                      style={{ background: '#FBF6E6', padding: '14px 18px', borderRadius: 6, boxShadow: '0 8px 18px rgba(70,55,28,.12)', borderLeft: '4px solid #C94E2C', display: 'flex', alignItems: 'center', gap: 14, transform: `rotate(${rots[i] ?? 0}deg)` }}
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: 14.5, color: '#221f1b' }}>{person}</div>
                        <div style={{ ...mono, fontSize: 11, color: '#A8997A' }}>{cat ? `${cat} · ` : ''}{dateStr}</div>
                      </div>
                      <button style={{ ...mono, fontSize: 11, border: 'none', color: '#fff', background: '#C94E2C', borderRadius: 6, padding: '6px 13px', cursor: 'pointer' }}>
                        Recordar
                      </button>
                    </div>
                  );
        ...
        {/* RIGHT */}
        <div style={{ flex: 1, minWidth: 280, display: 'flex', flexDirection: 'column', gap: 34 }}>
          ...
          <section style={{ position: 'relative', display: 'flex', alignItems: 'flex-start' }}>
            ...
            <div className="mesa-lift" style={{ position: 'relative', zIndex: 2, width: 262, background: '#FFFDF7', padding: '18px 22px 34px', boxShadow: '0 14px 30px rgba(70,55,28,.2)', transform: 'rotate(-.7deg)', clipPath: 'polygon(0 0,100% 0,100% 93%,95% 100%,90% 93%,85% 100%,80% 93%,75% 100%,70% 93%,65% 100%,60% 93%,55% 100%,50% 93%,45% 100%,40% 93%,35% 100%,30% 93%,25% 100%,20% 93%,15% 100%,10% 93%,5% 100%,0 93%)', ...mono, color: '#2a2722' }}>
