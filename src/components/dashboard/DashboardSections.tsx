...
export function AccountsSection({
  ...
  accounts: DashboardSummary['accounts'] | undefined;
  loading: boolean;
}) {
  return (
    <section>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[1, 2].map(n => (
            <div key={n} className="section-frame rounded-[1.35rem] bg-dark-card/90 p-5">
...
function getBudgetTone(threshold: string) {
  switch (threshold) {
    case 'over_limit':
      return { bar: 'from-red-500 to-rose-400', text: 'text-red-300' };
    case 'warning':
      return { bar: 'from-amber-500 to-yellow-300', text: 'text-amber-300' };
    default:
      return { bar: 'from-emerald-500 to-emerald-300', text: 'text-emerald-300' };
  }
}
...
export function BudgetsSection({
  ...
  budgets: DashboardSummary['budgets'] | undefined;
  loading: boolean;
}) {
  return (
    <section>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {budgets.map(budget => {
              ...
              const tone = getBudgetTone(budget.threshold);
                ...
                <div key={budget.id}>
                  <div className="flex justify-between text-xs mb-2">
                    ...
                    <span className="text-dark-muted tabular-nums">
                      {fmt(budget.spent_amount)} / {fmt(budget.limit_amount)}
                    </span>
                  </div>
